...
glm::mat4 DirectionalLight::get_projection(float width, float height, float near, float far) const { // NOLINT(readability-convert-member-functions-to-static)
    return glm::ortho(-width, width, -height, height, near, far);
}

glm::mat4 DirectionalLight::get_view() const {
