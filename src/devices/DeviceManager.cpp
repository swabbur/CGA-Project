...
DeviceManager::DeviceManager() : handle(nullptr), keyboard(), mouse(), window(), running(false), thread() {
    ...
    thread = std::thread([this, &promise]() {
        ...
        glfwSetScrollCallback(static_cast<GLFWwindow *>(handle), [](GLFWwindow * /* window */, double x, double y) {
            device_manager->mouse.scroll(static_cast<float>(x), static_cast<float>(y));
        });
